...
The static project page is self-contained (no build step, all CSS/JS vendored under `static/`). Preview it locally with:
...
GitHub Pages can only serve the repository root or `/docs`, so publishing `docs/website/` requires a Pages workflow that uploads that subfolder as the artifact.
